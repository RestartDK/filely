#include "cli.h"
#include "formatters/bmpformat.h"
#include "formatters/jpegformat.h"
#include "formatters/pngformat.h"
#include "formatters/textformat.h"
#include "utils.h"
#include <cstdio>
#include <fstream>
...
TEST(CLIArgumentTest, ValidArguments) {
  int argc = 6;
  char *argv[] = {
      (char *)"file_converter", (char *)PATH, (char *)"-t",
      (char *)"jpeg",           (char *)"-p", (char *)"output.jpeg"};
  ...
  if (options.outputFormat == "jpeg") {
    ASSERT_NE(dynamic_cast<JPEGFormat *>(outputFormat.get()), nullptr)
        << "Expected JPEGFormat.";
  } else if (options.outputFormat == "png") {
    ASSERT_NE(dynamic_cast<PNGFormat *>(outputFormat.get()), nullptr)
        << "Expected PNGFormat.";
  }
  ...
  EXPECT_EQ(actualExtension, expectedExtension)
      << "Output file extension does not match desired output format.";
...
TEST(CLIArgumentTest, InvalidFormat) {
  int argc = 4;
  char *argv[] = {(char *)"file_converter", (char *)PATH, (char *)"-t",
                  (char *)"bmp"};
...
TEST(CLIArgumentTest, UnknownOption) {
  int argc = 5;
  char *argv[] = {
      (char *)"file_converter", (char *)PATH, (char *)"-x",
      (char *)"jpeg",           (char *)"-p", (char *)"output.jpeg"};
