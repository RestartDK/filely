...
// Pulsing animation
void pulsingAnimation(const std::string &art, int pulses = 3,
                      int delayMs = 200) {
...
CLIOptions parseArguments(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    resetConsoleColor();
    std::vector<std::string> validFormats = {"png",  "jpg", "jpeg", "csv",
                                             "json", "bmp", "txt"};
...
void displayHelp() {
  ...
  resetConsoleColor();
  std::cout << "Usage: file_converter <input_file> -t <output_format> -p "
               "<output_path>\n"
            << "Options:\n"
            ...
            << "\nIf no arguments are provided, the tool will start in "
               "interactive mode.\n";
}
